...
Instead of manually creating multiple RedCap&trade; fields to capture data for what is ultimately a single discrete value, use a custom type, and let this script do all the hard work. A list of all available custom datatypes can be found on the [wiki](https://github.com/cbmi/redcap-repeat/wiki/Custom-Datatypes). Let's see a few examples.
